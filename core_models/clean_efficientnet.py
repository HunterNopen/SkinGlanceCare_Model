...
class AdaptiveClinicalLoss(nn.Module):
    def __init__(self, class_counts=None, gamma=2.0, label_smoothing=0.1):
        super().__init__()
        self.gamma = gamma
        self.label_smoothing = label_smoothing
        
        weights = self._calculate_adaptive_weights(class_counts)
        self.alpha = weights.bfloat16().to('cuda')
    
    def _calculate_adaptive_weights(self, class_counts):
        beta = 0.999
        effective_num = 1.0 - torch.pow(beta, torch.tensor(class_counts))
...
class SkinGlanceCareClassifier(pl.LightningModule):
    def __init__(self, cfg: Config, class_counts = None):
        ...
        #self.loss_fn = ClinicalLoss(alpha=alpha_weights, gamma=2.5, label_smoothing=0.0)
...
def create_trainer(cfg: Config) -> pl.Trainer:
    ...
    trainer = pl.Trainer(
        ...
        logger=TensorBoardLogger('lightning_logs', name='skin_glance_care_classifier_22_11'),
        callbacks=callbacks,
...
def main(train_pipeline: bool = False, best_checkpoint: str = None):    
    ...
    if best_checkpoint and os.path.exists(best_checkpoint):
        ...
        img_path = "C:/Users/User/Downloads/self_photo1.jpg"
        ...
        plt.imshow(input_tensor.cpu().numpy().squeeze(0).transpose((1, 2, 0)))
        plt.show()

        print("Predicted class:", cfg.label_classes[pred_class])
